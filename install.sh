#!/usr/bin/env bash
# 把 AIComplete 链接进 Sublime Text 的 Packages 目录。
#
...
if [ "${1:-}" = "--copy" ]; then
    ...
    for item in ai_complete.py lib .python-version \
                AIComplete.sublime-settings Main.sublime-menu \
...
echo "  2. 命令面板搜 «AIComplete: 测试与服务的连通性» 确认后端通了"
echo "  3. 默认后端是本地 Ollama，改成别的请编辑："
echo "     Preferences → Package Settings → AIComplete → Settings"
